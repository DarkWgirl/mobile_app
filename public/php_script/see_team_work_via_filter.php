...
if($work_status == "ALL"){
	$get_writers_work = mysqli_query($pos_db, "SELECT * from article_tbl INNER JOIN writer_tbl on writer_tbl.writer_id = article_tbl.writer_id where article_submit_date = '$submit_date' ORDER BY article_submit_date DESC");

}else{
	$get_writers_work = mysqli_query($pos_db, "SELECT * from article_tbl INNER JOIN writer_tbl on writer_tbl.writer_id = article_tbl.writer_id where article_status = '$work_status' AND article_submit_date = '$submit_date' ORDER BY article_submit_date DESC");
