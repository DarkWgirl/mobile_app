...
$get_remark = mysqli_query($pos_db, "SELECT * FROM remark_tbl INNER JOIN writer_tbl on writer_tbl.writer_id = remark_tbl.writer_id where article_id = '$aid' ORDER BY remark_submit_date DESC");

while($row = mysqli_fetch_array($get_remark)){

	$data[] =  array("remark_date"=>$row['remark_submit_date'], "remark"=>$row['remark'], "editor"=>$row['writer_name'], "remark_id"=>$row['remark_id']);
