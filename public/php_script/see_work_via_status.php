...
if($work_status == 'ALL'){
	...
	$get_writers_work = mysqli_query($pos_db, "SELECT * from article_tbl where writer_id = '$writer_id' ORDER BY article_id DESC");

}else{

	$get_writers_work = mysqli_query($pos_db, "SELECT * from article_tbl where writer_id = '$writer_id' AND article_status = '$work_status' ORDER BY article_id DESC");
